...
export const AvailabilityCalendar = (props: Props) => {
        ...
        if (!props.isEditable) {
            toast.custom((_) => (
                <div class="bg-base-100 border border-base-300 p-4 rounded-lg shadow-xl text-sm pointer-events-auto z-50">
                    ...
                    <p>👤 {eventProps.trainerName || "Unknown Trainer"}</p>
                    <p>
                        🕒{" "}
                    ...
                    <span class={`badge badge-sm mt-2 ${eventProps.is_booked ? "badge-error text-white" : "badge-success text-white"}`}>
                        {eventProps.is_booked ? "Booked" : "Available"}
                    </span>
                </div>
            ));
            return;
    ...
    const handleEventDrop = async (info: EventDropArg | EventResizeDoneArg) => {
        ...
        try {
            ...
            toast.success("Availability moved!");
        } catch (e) {
            console.error(e);
            revert();
            toast.error("Could not move availability.");
        }
    };
