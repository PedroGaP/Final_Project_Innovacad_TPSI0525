...
const Schedules = () => {
  ...
  const [selectedTrainerEmail, setSelectedTrainerEmail] =
    createSignal<string>("");
  const [startDate, setStartDate] = createSignal<string>("");
  const [endDate, setEndDate] = createSignal<string>("");
  const [searchQuery, setSearchQuery] = createSignal("");

  const [unusableClassesData] = createResource<Class[]>(api.fetchClassesPublic);
  const usableClassesData = createMemo(() => {
    const data = unusableClassesData();

    if (!data) return [];

    return data.map((element) => ({
      ...element,
      value_for_select: element.identifier,
      display_text: `${element.identifier} - ${element.location}`,
    }));
  });

  const [trainersData] = createResource(api.fetchTrainersPublic);
  const [unusableSchedulesData] = createResource(api.fetchSchedulesPublic);

  const usableSchedulesData = createMemo(() => {
    const data = unusableSchedulesData();

    if (!data) return [];

    return data.map((element) => ({
      ...element,
      unique_key: self.crypto.randomUUID(),
    }));
  });

  // --- ALTERAÇÃO AQUI ---
  const filteredSchedules = createMemo(() => {
    let schedules = usableSchedulesData() || [];

    // Filtro APENAS por Formador (independentemente do viewMode)
    if (selectedTrainerEmail()) {
      schedules = schedules.filter(
        (s) => s.trainer_email === selectedTrainerEmail(),
      );
    }

    // O Filtro por Turma foi removido conforme pedido.
    ...
    <div class="min-h-screen bg-base-200 p-6">
      <div class="w-full">
        <div class="mb-8">
          ...
          <p class="text-base-content/60">{t("public.schedules.desc")}</p>
        </div>
          ...
          <div class="card-body">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div class="form-control">
                <label class="label">
                  <span class="label-text font-semibold">
                    {t("public.schedules.select_trainer")}
                  </span>
                </label>
                <select
                  ...
                  onChange={(e) => {
                    setSelectedTrainerEmail(e.currentTarget.value);
                  }}
                >
                  <option value="">
                    {t("public.schedules.select_all_trainers")}
                  </option>
              ...
              <div class="form-control">
                <label class="label">
                  <span class="label-text font-semibold">
                    {t("public.schedules.search_label")}
                  </span>
                </label>
                <div class="relative">
                  <Icon
                  ...
                  <input
                    type="text"
                    placeholder={t(
                      "public.schedules.search_detailed_placeholder",
                    )}
            ...
            <div class="flex justify-end mt-4">
              <button
                ...
                onClick={() => {
                  ...
                  setEndDate("");
                  setSearchQuery("");
                  setSelectedTrainerEmail(""); // Limpar também o trainer
                  setSelectedClass("");
                }}
              >
                <Icon name="X" size={16} />
                          ...
                          <tr class="hover">
                            ...
                            <td>
                              ...
                              <div class="text-xs text-base-content/60">
                                {schedule.current_duration?.toFixed(1)}h /{" "}
                                {schedule.total_duration?.toFixed(1)}h
                              </div>
                            </td>
                            <td>
                              <div class="flex items-center gap-2">
